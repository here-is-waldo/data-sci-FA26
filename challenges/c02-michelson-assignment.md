...
*ANS: Count and distinctness match exactly. The mean velocities are
fairly close (+- 2 km/s). Decimal velocities do not make sense in this
context since the speed of light defines the meter.*

Why might your table differ from Michelson’s?

*ANS: Notably, Michelson’s values are all integers and multiples of 10.
Two of the mean velocities in my table have decimal components. This may
be due to the number of values averaged to calculate each mean velocity,
rounding, or perhaps another process.*
...
df_q2 <- df_michelson %>% mutate(VelocityVacuum = Velocity + 92)
...
true_err <- LIGHTSPEED_VACUUM - LIGHTSPEED_MICHELSON  ## Calc. actual error
LIGHTSPEED_PM > abs(true_err)  ## Is Michelson's error greater than the true error? 
```
...
*ANS: Michelson’s uncertainty is less than the true error. In other
words, the true speed of light is not contained within Michelson’s
uncertainty interval.*

Make a quantitative comparison between Michelson’s uncertainty and his
error.

*ANS: Michelson’s uncertainty is 51 km/s. His error was ~-151 km/s,
which is 2.97 times the magnitude of his estimated error (uncertainty)
of 51 km/s.*
...
- Both have a number of values outside of the uncertainty interval

- Both have daily averages outside of the uncertainty interval

- Both have a series of daily averages within the uncertainty from June
  12-20

- The daily averages outside of the uncertainty interval have 4 or less
  data points

Differences

- The real data has a greater range in measured velocities than the
  simulated data

- The simulated data has daily averages closer to the real speed of
  light
...
- Not all level 3 (“good) distinctness measurements are close to (within
  the uncertainty interval) the true speed of light
- There are multiple levels of distinctness within measurements from a
  single day
- The day with all level 1 (“poor”) distinctness has higher accuracy
  than most days with only level 3 observations.

``` r
df_q2 %>%
  ggplot(aes(Date, VelocityVacuum, color = Temp)) +
  geom_point() +
  geom_hline(
    yintercept = LIGHTSPEED_MICHELSON,
    linetype = "dotted"
...
- The three most outlying data points (in terms of error) are at lower
  temperatures
- The most accurate measurements are across a range of temperatures
- Error does not appear to scale with temperature
- The temperature ranges ~30 degrees Fahrenheit
- Error does not appear to directly relate to either temperature or
  distinctness
